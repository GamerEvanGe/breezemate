...
def _system_prompt(src_lang: str, tgt_lang: str) -> str:
    return (
        ...
        "\n"
        "Continuation handling (important):\n"
        "  * Previous turns in this conversation are real prior speech\n"
        "    from the same speaker, often chopped at a forced timeout\n"
        "    rather than a natural pause. If the current input clearly\n"
        "    continues the previous turn's sentence (no opening capital,\n"
        "    starts with 'and / but / so / because / 然后 / 但是 / ...'),\n"
        "    treat it as a continuation: polish and translate it as a\n"
        "    standalone fragment that flows from the previous turn, but\n"
        "    DO NOT repeat the previous turn's content in your output.\n"
        "  * Use the previous turn ONLY to disambiguate pronouns,\n"
        "    pick consistent translations for repeated terms, and\n"
        "    decide whether to capitalise the first character of the\n"
        "    polished output.\n"
        "\n"
